...
global $USER;
###
\Bitrix\Main\Loader::includeModule('miet.trade');
use MIET\Trade;
...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"] === "N"
    ...
    ###
    ###Получение списка показателей Trade для сотрудника###
    $arResult['ITEMS'] =
        Trade\TradeManager::GetTradeEmployee($arParams["USER_ID"]);


    foreach($arResult['ITEMS'] as &$arTrade) {
        $arValue = Trade\TradeManager::GetTradeEmployeeValue($arTrade["ID"], $arParams["USER_ID"], $_REQUEST['UF_PERIOD']);
        $arTrade["Trade_ID"] = $arValue["ID"];
        $arTrade["Trade_VALUE"] = $arValue["UF_VALUE"];
    }

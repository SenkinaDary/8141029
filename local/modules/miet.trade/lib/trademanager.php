...
class TradeManager
{
    ...
    {
        $elements = array();
        //Получаем ID инфоблока Trade по его символьному коду
        $rsIblock = \CIBlock::GetList(
            array(),
            array('CODE' => self::IBLOCK_CODE_Trade, 'SITE_ID' =>
                SITE_ID)
        );
    ...
    public static function GetTradeEmployee($idEmployee)
    {
        if (!$idEmployee) {
            return array();
        }
        ...
        //Получаем список всех Trade данных подразделений
    ...
    public static function SetTradeEmployee($idEmployee, $period,
                                          $arTradeValues)
    {
        if (!$idEmployee || !is_array($arTradeValues) ||
            !count($arTradeValues)
        ) {
            return array();
        }
        ...
        foreach ($arTradeValues as $Trade => $TradeValue) {
            $arValue = array(
                'UF_VALUE' => $TradeValue,
                'UF_TRADE' => $Trade,
    ...
    public static function GetTradeEmployeeValue($idTrade, $idEmployee, $period)
    {
        return TradeEmployeeTable:: getList(array(
